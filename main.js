...
slack.on(RTM_EVENTS.MESSAGE, (message) => {
    ...
    if (message.text) {
        ...
        //Check string agains local library.
        require('cld').detect(msg, options, function(err, result) {
          //if CLD is unable to determine language or matched language does not exist in whitelist.
          //Send msg to Google for futher investiagtion
          if ((!result) || (config.ignorelanguages == false) || (!config.ignorelanguages.includes(result['languages'][0]['code']))){
